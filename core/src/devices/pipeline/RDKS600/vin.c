...
/* VCON buses, from the platform's S600 config for this sensor; the stack does not derive
 * them from the device tree, so they are stated here. */
#define GS130_VIN_VCON_BUS_MAIN   2
#define GS130_VIN_VCON_BUS_SECOND 2

/* Number of driver-held capture buffers for the VIN output; the platform's S600
             ...
             uint32_t width, uint32_t height, uint32_t fps)
{
    ...
    attr.vin_node_attr.cim_attr.func.set_init_frame_id = 1;
    attr.vin_node_attr.cim_attr.func.enable_pattern    = 0;   /* test pattern off */
    /* No LPWM channel is selected as the frame-id trigger source: the exposure trigger is
       generated by the lpwm_attr block below, not by a channel fed back into the CIM. */
    attr.vin_node_attr.cim_attr.func.lpwm_trig_sel = (int32_t)LPWM_CHN_INVALID;

    attr.vin_node_attr.cim_attr.rdma_input.rdma_en = 0;

    attr.vin_node_attr.magicNumber = GS130_S600_MAGIC_NUMBER;
    ...
    for (uint32_t ch = 0; ch < 4; ch++) {
        ...
        attr.vin_node_attr.lpwm_attr.lpwm_chn_attr[ch].duty_time      = 100;
    ...
    attr.vin_ochn_attr[VIN_MAIN_FRAME].magicNumber   = GS130_S600_MAGIC_NUMBER;

    attr.vin_ochn_buff_attr[VIN_MAIN_FRAME].buffers_num = GS130_VIN_BUF_NUM;

    attr.magicNumber = GS130_S600_MAGIC_NUMBER;
