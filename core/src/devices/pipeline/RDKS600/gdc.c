 ...
 * built in RDKS600.cpp: an array of RemapPoint{double x, y} entries, reinterpreted here
...
/* GDC strides are 16-byte aligned on S600; the platform's camera stack aligns them the
             ...
             uint32_t grid_w, uint32_t grid_h, int install_angle)
{
    ...
    /* S600 hands the buffer back as void*, X5 as uint32_t*; the mapping itself is built
    ...
    settings.magicNumber              = GS130_S600_MAGIC_NUMBER;
