/**
 * @file RDKS600.cpp
 * @brief RDK S600 pipeline: camera probe/init, VIN-ISP-PYM/GDC flow setup, frame fetch.
 *
 * Backend of gs130::pipeline::Pipeline for the RDK S600 / Horizon driver stack, built
 * on the node helpers declared in RDKS600.h.
 *
 ...
 */
#include "devices/pipeline/RDKS600/RDKS600.h"
...
// S600 resource identifiers; see the file comment for where these come from. The platform's
// sensor tables use ISP/PYM hw_id 3 for a directly connected sensor, and 0 for the GMSL ones.
constexpr uint32_t kIspHwId = 3;
constexpr uint32_t kPymHwId = 3;

// First ISP/PYM slot id. The platform hands these out from isp0_next_slot_id, which starts
// at 4, and its sensor table for this sensor carries slot_id 4 too. Starting at 0 left the
// nodes unbound.
                           ...
                           uint32_t timeout_ms)
{
    ...
    /* ISP and PYM publish frame groups on S600. PYM groups its pyramid slots, while the
