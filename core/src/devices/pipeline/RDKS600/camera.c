                ...
                const char *tuning_file)
{
    /* MIPI receiver side: RAW10, one virtual channel, timing parameters handed in by the
       caller (see RDKS600.h). nocheck is enabled because the receiver is started by the
       sensor's own stream, as the platform's S600 configuration for this sensor does. */
    ...
    /* Sensor side; the exposure trigger is generated outside the sensor (LPWM, see vin_open()).
       ...
       and the addresses the platform's S600 table carries belong to its own module. */
    camera_config_t cam_cfg = {
        .name = "sc132gs",
        .addr = i2c_addr,
        .eeprom_addr = 0x51,   /* from the platform's S600 table for this sensor */
        .serial_addr = 0x40,
