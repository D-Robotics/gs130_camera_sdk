 ...
 *
 * Buffer counts, handshaking flags and the blanking values follow the platform's own
 * configuration for this sensor (multimedia_samples/vp_sensors/sc132gs/
 * linear_1088x1280_raw10_30fps_2lane.c).
 *
...
#define GS130_PYM_SUFFIX_HB      100
#define GS130_PYM_PREFIX_HB      2
#define GS130_PYM_SUFFIX_VB      10
#define GS130_PYM_PREFIX_VB      0
             ...
             uint32_t out_w, uint32_t out_h, uint32_t hw_id, uint32_t slot_id)
{
    ...
    /* Node identity. The mode is M2M, not the MANUAL mode the platform's own S600 sensor
       table carries: this pipeline feeds PYM from the ISP's DDR output (offline), and a
       MANUAL node refuses that bind -- the kernel logs "vpf_check_allow_bind: allow_bind
       btype(2) failed" and init fails with HwError. Measured on the board; see
       review/jira015-g3-plan.md v2. The layer each output reads is chosen here through
       ds_roi_* below, so nothing is left for the node to derive. */
    cfg.hw_id    = hw_id;
    cfg.slot_id  = slot_id;
    cfg.pym_mode = PYM_M2M_MODE;

    /* Buffer counts and handshaking, as in the platform's configuration. */
    cfg.pingpong_ring        = 0;
    ...
    cfg.magicNumber = GS130_S600_MAGIC_NUMBER;
