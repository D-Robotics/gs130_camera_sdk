/**
 * @file gs130-detect-imu.cpp
 * @brief Scan I2C buses x addresses for a known IMU.
 *
 * Internal bring-up tool, dispatched by the @c gs130 script as
 * @c "gs130 detect imu": every (bus, address) probe point prints exactly one
 ...
 *
 * Usage: gs130 detect imu -b <bus...> [-a <addr...>]
 *
 ...
 * @return 0 when the scan completed (also when no IMU was found), 1 on a usage
 *         or argument error.
 */
int main(int argc, char **argv)
{
    ...
    if(buses.empty()){
        fprintf(stderr, "usage: gs130 detect imu -b <bus...> [-a <addr...>]   (addr default: 0x68 0x69)\n");
        return 1;
    }
    // default probe addresses: the two I2C addresses an ICM-family IMU can answer on
    if(addrs.empty())addrs = {0x68, 0x69};

    printf("IMU scan: buses");
    ...
    for(int b : buses){
        // bus-open sanity: if the first open fails, the bus node is missing
        ...
        for(int a : addrs){
            ...
            imu::Imu imu((uint8_t)b, (uint8_t)a);
            if(imu){
                printf(" %3d  0x%02x   %s\n", b, a, imu.name());
                print_block("                ", imu.info());
                n_found++;
            } else {
                // not a known IMU: distinguish "something answers" from "nothing there"
