...
``GS130_CONFIG_RDKX5_GS130W`` and its ``_NO_EEPROM`` variant have no
``.imu_fifo``, so that queue keeps the zeroed ``depth = 0`` and ``DROP_NEW``.
The SDK accepts that because its depth check only applies when
``imu_config.bus_num`` is non-zero.
"""
